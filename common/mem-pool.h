...
namespace Common{
    ...
    class MemoryPool final{
        public:
            MemoryPool(std::size_t num_elems){
                store(num_elems,{T(,true)}); // initialize store vector;
                // Make sure that the ObjectBlock's memory starts with object_.
                ASSERT(reinterpret_cast<ObjectBlock*>(&(store[0].object_)) = &(store[0]));
            }
            ...
            void updateNextFreeIndex(){
                const auto initial = next_free_index;
                    ...
                    if (UNLIKELY(initial_free_index == next_free_index_)) {
                        ASSERT(initial_free_index != next_free_index_, "Storage is FULL!!");
                    }
                }
            }
    }; 
